...
def data_save(request):
    ...
    response_android = 'NO2 '+str(avgno2) + ' Respiratory and Cardiovascular Illness  SO2: ' + str(avgso2) + ' Respiratory and Cardiovascular Illness  CO: ' +str(avgco)+' Headache And Fatigue  PM25: '+str(avgpm25)+'Lung Damage  Ozone: '+str(avgo3)+'Respiratort Illness '
...
def call_avg(dev_id):
    response= unirest.get("https://openenvironment.p.mashape.com/all/public/data/hours/"+str(24)+"/"+dev_id,
      headers={
        "X-Mashape-Key": "G2v9SeAYZDmshOQcIodXHnaRjpChp1CqdwLjsnMURaezdTxudI",
        "Accept": "application/json"
      })
    ...
    for i in range(0,24):
        ...
        if 'pm25' in jsonResponse[i]['payload']['d']:
            ...
            f=jsonResponse[i]['payload']['d']['pm25'];
            sumpm25=sumpm25+f
    return sumso2/24,sumno2/24,sumo3/24,sumco/24,sumpm25/24
...
def predictive_analytics(request):
    ...
    dev_id=request.data['d']
    avgs02,avgno2,avgo3,avgco,avgpm25=call_avg(dev_id)
    x_list=[]
    x_list.append(avgs02)
    x_list.append(avgno2)
    x_list.append(avgo3)
    x_list.append(avgco)
    x_list.append(avgpm25)
    ...
    #x_list.reshape(1, -1)
    pred_aqi=clf.predict(x_list)
    return HttpResponse(str(res)+"   "+str(pred_aqi))
''''
